...
from codepilot.teams.progress import TeammateProgress, random_verb

if TYPE_CHECKING:
    from codepilot.agent import Agent
    from codepilot.conversation import ConversationManager
    from codepilot.teams.models import TeammateInfo
...
async def _wait_for_next_prompt_or_shutdown(
    ...
    循环休眠并检查邮箱；收到 shutdown 消息返回 ("", True)，否则把普通
    消息拼成下一轮的 prompt。
    """
    while True:
    ...
    async def _run() -> str:
        ...
        try:
            if conversation is not None:
                ...
                from codepilot.conversation import ConversationManager as CM
