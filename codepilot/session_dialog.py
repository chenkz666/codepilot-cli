...
class InlineResumeWidget(Vertical, can_focus=True):
    ...
    def _build_content(self) -> str:
        ...
        for i, meta in enumerate(self._filtered[:10]):  # 最多显示 10 条
            ...
            if meta.message_count:
                parts.append(f"{meta.message_count} messages")
            if meta.total_tokens:
                parts.append(f"{meta.total_tokens:,} tokens")
    ...
    def _refilter(self) -> None:
        if not self._search:
            ...
            self._filtered = [
                m for m in self._sessions if s in (m.title or "").lower() or s in m.id.lower()
            ]
        self._cursor = 0
        self._refresh()
