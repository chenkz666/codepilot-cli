...
def resolve_agent_tools(
    ...
    if is_background:
        all_tools = {
            name: tool for name, tool in all_tools.items() if name in ASYNC_AGENT_ALLOWED_TOOLS
        }
    ...
    if definition.tools:
        ...
        all_tools = {name: tool for name, tool in all_tools.items() if name in allowed_set}
...
def build_teammate_tools(
    ...
    from codepilot.tools.send_message import SendMessageTool
    from codepilot.tools.task_create import TaskCreateTool
    from codepilot.tools.task_get import TaskGetTool
    from codepilot.tools.task_list import TaskListTool
    from codepilot.tools.task_update import TaskUpdateTool
    ...
    if definition is not None:
        ...
        if definition.tools:
            ...
            filtered = {name: tool for name, tool in filtered.items() if name in allowed_set}
...
def clone_registry_for_fork(parent_registry: ToolRegistry) -> ToolRegistry:
    ...
    from codepilot.tools.agent_tool import FORK_QUERY_SOURCE
