...
from codepilot.memory.instructions import process_includes

log = logging.getLogger(__name__)

# 不限制模型白名单；第三方模型名称（如 "glm-5.1"）直接交给主机的
# ModelResolver 或 LLM 路由判断实际可用性。
...
def _validate_agent_meta(meta: dict, source: str = "") -> None:
    ...
    if pm not in VALID_PERMISSION_MODES:
        raise AgentParseError(
            f"Invalid permissionMode '{pm}'{ctx}: must be one of {VALID_PERMISSION_MODES - {''}}"
        )
    ...
    if isolation not in VALID_ISOLATION_MODES:
        raise AgentParseError(
            f"Invalid isolation '{isolation}'{ctx}: must be one of {VALID_ISOLATION_MODES - {''}}"
        )
