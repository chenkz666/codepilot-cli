...
from codepilot.worktree.changes import (
    CleanupResult,
    count_worktree_changes,
    has_worktree_changes,
)
from codepilot.worktree.models import Worktree, WorktreeSession
from codepilot.worktree.session import load_worktree_session, save_worktree_session
from codepilot.worktree.setup import perform_post_creation_setup
...
class WorktreeManager:
    def __init__(
        ...
        self.worktree_dir = worktree_dir or str(Path(repo_root) / ".codepilot" / "worktrees")
        self._codepilot_dir = Path(repo_root) / ".codepilot"
    ...
    async def enter(self, name: str) -> WorktreeSession:
        ...
        save_worktree_session(self._codepilot_dir, session)
        return session
    ...
    async def exit(
        ...
        save_worktree_session(self._codepilot_dir, None)
    ...
    def restore_session(self) -> WorktreeSession | None:
        session = load_worktree_session(self._codepilot_dir)
        if session is None:
            return None
