...
def _session_path(codepilot_dir: Path) -> Path:
    return codepilot_dir / SESSION_FILENAME
...
def save_worktree_session(
    ...
    path = _session_path(codepilot_dir)
    if session is None:
...
def load_worktree_session(codepilot_dir: Path) -> WorktreeSession | None:
    path = _session_path(codepilot_dir)
    if not path.exists():
        return None
    try:
