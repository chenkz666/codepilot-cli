...
def load_config(path: Path | None = None) -> AppConfig:
    ...
    home = Path.home()
    candidates = [
        home / ".codepilot" / "config.yaml",
        cwd / ".codepilot" / "config.yaml",
        cwd / ".codepilot" / "config.local.yaml",
    ]
    ...
    if merged is None:
        raise ConfigError(
            "No config file found. Expected .codepilot/config.yaml "
            "in project or ~/.codepilot/config.yaml"
        )
    return merged
