...
class InstallSkillTool(Tool):
    """从 URL 下载并安装 skill 到用户全局目录 (~/.codepilot/skills/)。
    ...
    description = (
        ...
        "(~/.codepilot/skills/). Supports skills.sh URLs (https://www.skills.sh/<owner>/<repo>/<name>), "
    ...
    async def execute(self, params: BaseModel) -> ToolResult:
        if not isinstance(params, InstallSkillParams):
            return ToolResult(output="Invalid InstallSkill parameters", is_error=True)

        from codepilot.skills.install import parse_skill_url, install_skill
