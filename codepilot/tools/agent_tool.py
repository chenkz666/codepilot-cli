...
if TYPE_CHECKING:
    ...
    from codepilot.agents.parser import AgentDef
    ...
    from codepilot.client import LLMClient
...
class AgentTool(Tool):
    ...
    async def execute(self, params: BaseModel) -> ToolResult:
        ...
        from codepilot.agents.fork import ForkError, build_forked_messages
        from codepilot.agents.parser import AgentDef
        from codepilot.agents.tool_filter import clone_registry_for_fork, resolve_agent_tools
        from codepilot.agent import Agent as AgentClass
        ...
        # 构建子 agent 工具注册表
        _base_registry = (
            getattr(self._parent_agent, "_full_registry", None) or self._parent_agent.registry
        )
        if is_fork:
        ...
        if p.subagent_type is None:
            from codepilot.context import clone_replacement_state
    ...
    async def _execute_as_teammate(self, p: AgentToolParams) -> ToolResult:
        ...
        if not p.team_name:
            return ToolResult(output="team_name is required for teammate spawn.", is_error=True)
        team_name = p.team_name

        from codepilot.agents.fork import ForkError, build_forked_messages
        from codepilot.agents.parser import AgentDef
        from codepilot.agents.tool_filter import build_teammate_tools
        from codepilot.agent import Agent as AgentClass
        from codepilot.conversation import ConversationManager
        from codepilot.permissions import (
            DangerousCommandDetector,
        ...
        from codepilot.teams.models import BackendType, TeammateInfo
        from codepilot.teams.registry import AgentNameRegistry

        team = self._team_manager.get_team(team_name)
        if team is None:
            return ToolResult(
                output=f"Team '{team_name}' not found. Create it first with TeamCreate.",
                is_error=True,
            )
        ...
        self._team_manager.register_member(team_name, member)

        # 8. 按后端类型启动队友
        if backend in (BackendType.TMUX, BackendType.ITERM2):
            return self._spawn_pane_teammate(
                p, team, member, backend, wt, agent_id, teammate_name, team_name
            )
    ...
    def _spawn_pane_teammate(
        ...
        from codepilot.teams.models import BackendType
        ...
        try:
            if backend == BackendType.TMUX:
                from codepilot.teams.spawn_tmux import spawn_tmux_teammate
                ...
                from codepilot.teams.spawn_iterm2 import spawn_iterm2_teammate
    ...
    async def _execute_with_worktree(self, p: AgentToolParams) -> ToolResult:
        ...
        from codepilot.worktree.integration import (
