...
from codepilot.hooks import HookConfigError, HookEngine, load_hooks
from codepilot.permissions import PermissionMode


def _configure_prompt_skills(registry, agent, work_dir: str):
    """Load project skills and expose them to a non-interactive agent."""
...
def main() -> None:
    # 先确保 .codepilot/ 目录存在，否则下面写 debug.log 会因目录不存在而崩溃
    Path(".codepilot").mkdir(parents=True, exist_ok=True)
    logging.basicConfig(
        level=logging.INFO,
        format="%(asctime)s %(name)s %(message)s",
        filename=".codepilot/debug.log",
        filemode="w",
    )

    parser = argparse.ArgumentParser(prog="codepilot", description="CodePilot AI coding assistant")
    parser.add_argument(
        "--mode",
    ...
    parser.add_argument(
        "--remote-host",
        default="127.0.0.1",
        help="Remote UI bind host (use 0.0.0.0 only on a trusted network)",
    )
    parser.add_argument(
        "--remote-port",
        type=int,
        default=18888,
        help="Remote UI bind port (default: 18888)",
    )
    ...
    if args.remote:
        from codepilot.remote import RemoteServer

        server = RemoteServer(
            providers=config.providers,
            mcp_servers=config.mcp_servers,
            hook_engine=hook_engine,
            addr=args.remote_host,
    ...
    from codepilot.driver import NoAltScreenDriver
...
async def _run_prompt(
    ...
    from codepilot.client import create_client, resolve_context_window
    from codepilot.conversation import ConversationManager
    from codepilot.memory.instructions import load_instructions
    ...
    from codepilot.tools import create_default_registry
    ...
    from codepilot.worktree import WorktreeManager
    ...
    _configure_prompt_skills(registry, agent, work_dir)
