...
from codepilot.client import create_client
from codepilot.commands import CommandContext, CommandRegistry
...
from codepilot.commands.parser import parse_command
...
from codepilot.hooks import HookEngine
from codepilot.mcp import MCPManager
from codepilot.memory import MemoryManager, load_instructions
from codepilot.memory.session import Session, SessionManager
...
from codepilot.web_content import INDEX_HTML
...
class RemoteServer:
    ...
    def _init_agent(self) -> None:
        ...
        self.file_history = FileHistory(work_dir, self.session_id)
    ...
    async def _handle_user_message(self, content: str) -> None:
        ...
        if self.conversation is None or self.agent is None:
            ...
            await self._broadcast(
                {
                    "type": "error",
                    "data": {"message": "Agent is not initialized"},
                }
            )
            return
    ...
    async def _handle_slash_command(self, input_text: str) -> None:
        ...
        if not name:
            ...
            for command in self.command_registry.list_commands():
                lines.append(f"  /{command.name:<18} {command.description}")
            await self._broadcast(
                {
                    "type": "system",
                    "data": {"message": "\n".join(lines)},
    ...
    async def _render_restored_messages(self, messages: list[Message]) -> None:
        ...
        for message in messages:
            if message.tool_results or not message.content:
                continue
            event_type = "replay_user" if message.role == "user" else "replay_assistant"
            await self._broadcast(
                {
                    "type": event_type,
