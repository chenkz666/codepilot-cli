...
from codepilot.permissions.dangerous import DangerousCommandDetector, is_safe_command
from codepilot.permissions.modes import DecisionEffect, PermissionMode, mode_decide
from codepilot.permissions.rules import RuleEngine, extract_content
...
class PermissionChecker:
    ...
    def check(self, tool: Tool, arguments: dict[str, Any]) -> Decision:
        ...
        if self.sandbox_enabled and tool.category == "command":
            ...
            subcommands = [
                s.strip() for s in re.split(r"\s*(?:&&|\|\||[;|])\s*", content) if s.strip()
            ]
            if not subcommands:
                subcommands = [content]
            has_ask = False
    ...
    def _is_plan_file(self, target_path: str) -> bool:
        if not self.plan_file_path or not target_path:
            return ".codepilot/plans/" in target_path
        try:
