...
from codepilot.commands import (
    CommandContext,
    CommandRegistry,
    complete,
    parse_command,
...
from codepilot.commands.handlers import register_all_commands
from codepilot.config import MCPServerConfig, ProviderConfig
from codepilot.hooks import HookContext, HookEngine
from codepilot.conversation import ConversationManager, Message
from codepilot.mcp import ConnectResult, MCPManager
from codepilot.memory import (
    MemoryManager,
    Session,
    SessionManager,
...
from codepilot.agents.task_manager import TaskManager
from codepilot.agents.trace import TraceManager
from codepilot.agents.notification import inject_task_notifications
from codepilot.commands.handlers.tasks import create_tasks_command
from codepilot.skills.executor import SkillExecutor
from codepilot.skills.loader import SkillLoader
from codepilot.commands.handlers.skill_register import register_skill_commands
from rich.text import Text as RichText
from textual.theme import Theme
from codepilot.cache import FileCache
from codepilot.tools import ToolRegistry, create_default_registry
from codepilot.tools.agent_tool import AgentTool
from codepilot.tools.ask_user import AskUserEvent, AskUserTool
from codepilot.tools.impl.tool_search import ToolSearchTool
from codepilot.tools.install_skill import InstallSkillTool
from codepilot.tools.load_skill import LoadSkill
from codepilot.worktree.cleanup import start_stale_cleanup_task
from codepilot.worktree.manager import WorktreeManager
from codepilot.commands.handlers.worktree import create_worktree_command
from codepilot.teammate_tree import TeammateTree
from codepilot import __version__

import re

if TYPE_CHECKING:
    from codepilot.askuser_dialog import InlineAskUserWidget
    from codepilot.permission_dialog import InlinePermissionWidget
    from codepilot.plan_dialog import InlinePlanWidget
...
_SKIP_DIRS = {".git", "node_modules", ".venv", "__pycache__", ".codepilot", "build", ".gradle"}
...
class ChatInput(TextArea):
    ...
    def load_history(self, work_dir: str) -> None:
        self._history_file = Path(work_dir) / ".codepilot" / "history"
...
class CodePilotApp(App):
    ...
    def __init__(
        ...
        from codepilot.config import SandboxAppConfig
    ...
    def _make_banner(model: str = "", work_dir: str = "") -> RichText:
        ...
        t.append(f"CodePilot v{__version__}\n", style="color(242)")
    ...
    def on_mount(self) -> None:
        self.register_theme(_CODEPILOT_THEME)
        self.theme = "codepilot"
        if len(self.providers) == 1:
    ...
    def _select_provider(self, provider: ProviderConfig) -> None:
        ...
        checker = PermissionChecker(
            ...
            rule_engine=RuleEngine(
                user_rules_path=home / ".codepilot" / "permissions.yaml",
                project_rules_path=Path(work_dir) / ".codepilot" / "permissions.yaml",
                local_rules_path=Path(work_dir) / ".codepilot" / "permissions.local.yaml",
            ),
        ...
        if self._sandbox_cfg.enabled:
            from codepilot.sandbox import SandboxConfig, create_sandbox
            ...
            if os_sandbox and os_sandbox.available():
                sandbox_config = SandboxConfig(
                    ...
                    deny_write=[
                        f"{work_dir}/.codepilot/config.yaml",
                        f"{work_dir}/.codepilot/permissions.local.yaml",
                    ],
        ...
        from codepilot.filehistory import FileHistory
        ...
        from codepilot.tools.exit_plan_mode import ExitPlanModeTool
        ...
        self._exit_plan_tool._is_plan_mode = lambda: agent.plan_mode
        self._exit_plan_tool._plan_exists = lambda: agent._get_plan_path().exists()
        ...
        register_skill_commands(self.command_registry, skill_loader, self.skill_executor)
        ...
        # --- Worktree 系统初始化 ---
        from codepilot.config import WorktreeConfig
        ...
        if restored:
            agent.set_work_dir(restored.worktree_path)

        wt_command = create_worktree_command(self.worktree_manager)
        self.command_registry.register_sync(wt_command)

        from codepilot.tools.enter_worktree import EnterWorktreeTool
        from codepilot.tools.exit_worktree import ExitWorktreeTool

        self.registry.register(
            EnterWorktreeTool(
                worktree_manager=self.worktree_manager,
                on_work_dir_changed=agent.set_work_dir,
            )
        )
        self.registry.register(
            ExitWorktreeTool(
        ...
        # --- Agent 团队系统初始化 ---
        from codepilot.teams.manager import TeamManager
        from codepilot.tools.team_create import TeamCreateTool
        from codepilot.tools.team_delete import TeamDeleteTool
        ...
        from codepilot.commands.handlers.trace import create_trace_command
        ...
        from codepilot.tools.synthetic_output import SyntheticOutputTool
    ...
    async def _prefetch_relevant_memories(self, query: str) -> str:
        ...
        async def selector(system_prompt: str, user_message: str) -> str:
            from codepilot.tools.base import StreamEnd, TextDelta
                ...
                if isinstance(event, ThinkingText):
                    ...
                    if self.session:
                        for msg in self.conversation.history[history_cursor:]:
                            self.session.append(msg)
                        history_cursor = len(self.conversation.history)
                        self.session.update_total_tokens(
    ...
    async def _process_task_notifications(self) -> None:
        ...
        self._agent_task = asyncio.create_task(self._send_message(""))
    ...
    def on_inline_plan_widget_responded(self, event: "InlinePlanWidget.Responded") -> None:
        from codepilot.plan_dialog import InlinePlanWidget, PlanChoice
        from codepilot.prompts import build_plan_mode_exit_reminder
    ...
    async def _init_mcp(self) -> None:
        ...
        if server_count > 0 and mcp_tools > 0:
            # 构建 MCP 指令：从 InitializeResult 提取服务端 instructions
            parts = []
