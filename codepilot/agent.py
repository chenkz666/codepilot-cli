...
from codepilot.conversation import ConversationManager, ToolResultBlock, ToolUseBlock
from codepilot.conversation import ThinkingBlock as ConvThinkingBlock
from codepilot.memory.auto_memory import MemoryManager
from codepilot.permissions import (
    PermissionChecker,
    PermissionMode,
)
from codepilot.hooks import HookContext, HookEngine
from codepilot.prompts import (
...
class Agent:
    def __init__(
        ...
        if memory_manager is not None:
            ...
            self._consolidator = MemoryConsolidator(self.work_dir)
        self.session_id: str = ""
        ...
        # 相对路径工具必须和 Agent 使用同一个工作目录。否则进入 worktree 后，
        # Bash / ReadFile / Glob 等工具仍会在启动目录执行。
    ...
    def _transcript_path(self) -> str:
        if self.session_id:
            return str(Path(self.work_dir) / ".codepilot" / "sessions" / f"{self.session_id}.jsonl")
        return ""
    ...
    def _get_plan_path(self) -> Path:
        ...
        plans_dir = Path(self.work_dir) / ".codepilot" / "plans"
    ...
    def reset_session_state(self) -> None:
        """清理不能跨会话继承的 Agent 状态。"""
        recall_task = self.memory_recall_task
    ...
    def set_work_dir(self, work_dir: str) -> None:
        """切换 Agent 及其工具、路径权限检查器使用的工作目录。"""
        resolved = str(Path(work_dir).resolve())
        self.work_dir = resolved
        self._plan_path_cache = None
        for tool in self.registry.list_tools():
            tool.work_dir = resolved
        if self.permission_checker is not None:
            from codepilot.permissions import PathSandbox

            self.permission_checker.sandbox = PathSandbox(resolved)
    ...
    def _build_hook_context(self, event: str, **kwargs: str | dict) -> HookContext:
        raw_tool_args = kwargs.get("tool_args", {})
    ...
    async def run(self, conversation: ConversationManager) -> AsyncIterator[AgentEvent]:
        ...
        while True:
            ...
            if isinstance(compact_result, CompactEvent):
                ...
                # 压缩后重新应用 budget（就地修改）
                apply_tool_result_budget(conversation, self.session_dir, self.replacement_state)
            ...
            if not response.tool_calls:
                ...
                if self.file_history is not None:
                    summary = (
                        response.text[:60] + "..." if len(response.text) > 60 else response.text
                    )
        ...
        if self.permission_checker:
            ...
            if decision.effect == "ask":
                ...
                if response == PermissionResponse.ALLOW_ALWAYS:
                    from codepilot.permissions.rules import Rule, extract_content
    ...
    def _maybe_persist_or_truncate(self, tool_use_id: str, text: str) -> str:
        from codepilot.context.manager import (
