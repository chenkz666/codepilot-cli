...
class CompletionPopup(Static):
    ...
    def _refresh_content(self) -> None:
        ...
        elif self._cursor >= self._window_start + self.MAX_VISIBLE_ITEMS:
            self._window_start = self._cursor - self.MAX_VISIBLE_ITEMS + 1

        end = min(
            len(self._displays),
            self._window_start + self.MAX_VISIBLE_ITEMS,
        )
        lines = []
        for i in range(self._window_start, end):
