...
from codepilot.memory.auto_memory import ENTRYPOINT_NAME, MAX_ENTRYPOINT_LINES
from codepilot.memory.session import SessionManager
...
class MemoryConsolidator:
    ...
    def __init__(
        ...
        *,
        ...
        self._mem_dir = os.path.join(work_dir, ".codepilot", "memory")
        self._user_mem_dir = os.path.join(Path.home(), ".codepilot", "memory")
    ...
    async def maybe_run(
        ...
        # 后台执行
        asyncio.ensure_future(self._run(client, conversation, protocol, session_ids, prior_mtime))
    ...
    async def _run(
        ...
        try:
            ...
            logger.debug("[consolidation] failed, rolling back lock", exc_info=True)
    ...
    async def _do_consolidation(
        ...
        transcript_dir = os.path.join(self._work_dir, ".codepilot", "sessions")
        ...
        checker = PermissionChecker(
            ...
            sandbox=PathSandbox(self._work_dir),
...
def _try_acquire_lock(mem_dir: str) -> int | None:
    ...
    # 仅删除刚才检查过的失效锁。再次核对 mtime，避免误删另一个进程
    # 在检查窗口中刚创建的新锁。
