...
def get_auto_mem_path(project_root: str) -> str:
    """返回项目级记忆目录路径：<projectRoot>/.codepilot/memory/。

    保留尾部分隔符，确保前缀匹配不会误命中类似 memoryxyz 的路径。
    支持 CODEPILOT_REMOTE_MEMORY_DIR 环境变量覆盖。
    """
    override = os.environ.get("CODEPILOT_REMOTE_MEMORY_DIR", "")
    if override:
        return override.rstrip(os.sep) + os.sep
    abs_root = os.path.abspath(project_root)
    return os.path.join(abs_root, ".codepilot", "memory") + os.sep


def get_user_auto_mem_path() -> str:
    """返回用户级记忆目录路径：~/.codepilot/memory/。
    ...
    if not home:
        return ""
    return os.path.join(home, ".codepilot", "memory") + os.sep
...
def build_memory_prompt(user_mem_dir: str, project_mem_dir: str) -> str:
    ...
    组合类型化记忆行为指令和两个 MEMORY.md 的内容，生成完整的
    '# auto memory' 系统提示段。
    """
...
class MemoryManager:
    ...
    使用独立 .md 文件、frontmatter 和 MEMORY.md 索引。
    ...
    def __init__(self, project_root: str) -> None:
        ...
        # 用户级：~/.codepilot/memory/ — user/feedback 类型记忆
        self._user_mem_dir = get_user_auto_mem_path()
        # 项目级：<projectRoot>/.codepilot/memory/ — project/reference 类型记忆
    ...
    def user_path(self) -> Path:
        ...
        return Path.home() / ".codepilot" / "memory" / ENTRYPOINT_NAME
    ...
    def user_mem_dir(self) -> Path:
        """用户级记忆目录（~/.codepilot/memory/）。"""
        return (
            Path(self._user_mem_dir.rstrip(os.sep))
            if self._user_mem_dir
            else Path.home() / ".codepilot" / "memory"
        )

    @property
    def project_mem_dir(self) -> Path:
        """项目级记忆目录（<project>/.codepilot/memory/）。"""
