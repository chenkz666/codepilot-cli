...
def _parse_include(trimmed: str) -> str:
    ...
    支持 @./path、@../path、@~/path、@/path 语法，以及旧格式
    "@include path"。返回空字符串表示该行不是 include 指令。
    """
    # 旧格式兼容：@include <path>
...
def _resolve_include(path: str, base_dir: Path) -> Path:
    ...
    将 ~/ 展开为 home，并基于 base_dir 解析相对路径。
    """
    if path.startswith("~/"):
    ...
    for line in lines:
        ...
        if not in_code:
            ...
            if include_path:
                ...
                result.append(
                    process_includes(included, resolved.parent, project_root, depth + 1, seen)
                )
                continue
...
def load_instructions(project_root: str) -> str:
    ...
    发现顺序（低优先级在前，高优先级在后）：
    1. 用户全局：~/.codepilot/CODEPILOT.md, ~/.codepilot/AGENTS.md
    2. 项目目录链：从 git root 到 workDir，每个目录的 CODEPILOT.md 和 AGENTS.md
    3. workDir/.codepilot/INSTRUCTIONS.md（遗留格式）
    4. workDir/CODEPILOT.local.md（本地覆盖）
    """
    ...
    # 1. 用户全局
    _add(home / ".codepilot" / "CODEPILOT.md")
    _add(home / ".codepilot" / "AGENTS.md")
    ...
    # 3. 遗留格式
    _add(root / ".codepilot" / "INSTRUCTIONS.md")

    # 4. 本地覆盖
    _add(root / "CODEPILOT.local.md")
