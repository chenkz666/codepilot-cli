...
需要环境变量 CODEPILOT_TEST_API_KEY、CODEPILOT_TEST_BASE_URL、CODEPILOT_TEST_MODEL。
...
def test_lock_first_acquire():
    from codepilot.memory.consolidation import _read_last_consolidated_at, _try_acquire_lock
...
def test_lock_blocks_when_held():
    from codepilot.memory.consolidation import _try_acquire_lock
...
def test_lock_is_atomic_across_concurrent_callers():
    from codepilot.memory.consolidation import _try_acquire_lock

    with tempfile.TemporaryDirectory() as d:
        with ThreadPoolExecutor(max_workers=8) as pool:
            results = list(pool.map(lambda _index: _try_acquire_lock(d), range(8)))
...
def test_process_running_detects_current_process():
    from codepilot.memory.consolidation import _is_process_running
...
def test_lock_rollback_deletes_on_zero():
    from codepilot.memory.consolidation import _try_acquire_lock, _rollback_lock
...
def test_prompt_contains_all_phases():
    from codepilot.memory.consolidation import _build_consolidation_prompt
...
def test_list_sessions_since_compares_datetime_timestamps(monkeypatch):
    from codepilot.memory import consolidation
    from codepilot.memory.session import SessionMeta

    now = datetime.now(timezone.utc)
    sessions = [
        SessionMeta(id="old", last_active=now - timedelta(hours=2)),
        SessionMeta(id="new", last_active=now),
    ]
    monkeypatch.setattr(
        consolidation,
        "SessionManager",
        lambda _work_dir: SimpleNamespace(list=lambda: sessions),
    )

    since_ms = int((now - timedelta(hours=1)).timestamp() * 1000)
    assert consolidation._list_sessions_since(".", since_ms) == ["new"]


@pytest.mark.asyncio
async def test_consolidator_builds_current_agent_interfaces(tmp_path, monkeypatch):
    import codepilot.agent as agent_module
    from codepilot.memory.consolidation import MemoryConsolidator
    from codepilot.permissions import PermissionMode
    ...
    await consolidator._do_consolidation(object(), object(), "openai-compat", [])

    assert captured["permission_checker"].mode == PermissionMode.BYPASS
    assert {tool.name for tool in captured["registry"].list_tools()} == {
        "ReadFile",
        "WriteFile",
        "EditFile",
        "Bash",
...
@pytest.mark.skipif(
    not os.environ.get("CODEPILOT_TEST_API_KEY"), reason="CODEPILOT_TEST_API_KEY not set"
)
@pytest.mark.timeout(120)
def test_e2e_consolidation_merges_duplicates():
    api_key = os.environ["CODEPILOT_TEST_API_KEY"]
    base_url = os.environ.get("CODEPILOT_TEST_BASE_URL", "https://api.minimaxi.com/v1")
    model = os.environ.get("CODEPILOT_TEST_MODEL", "MiniMax-M3")

    with tempfile.TemporaryDirectory() as work_dir:
        mem_dir = os.path.join(work_dir, ".codepilot", "memory")
...
async def _run_consolidation(work_dir, api_key, base_url, model, mem_dir):
    ...
    from codepilot.permissions.checker import PermissionChecker
    from codepilot.tools import ToolRegistry
    from codepilot.tools.bash import Bash
    from codepilot.tools.edit_file import EditFile
    from codepilot.tools.glob import Glob
    from codepilot.tools.grep import Grep
    from codepilot.tools.read_file import ReadFile
    from codepilot.tools.write_file import WriteFile

    from codepilot.config import ProviderConfig
    from codepilot.client import OpenAICompatClient
    ...
    from codepilot.permissions.rules import RuleEngine
    from codepilot.permissions.dangerous import DangerousCommandDetector
    from codepilot.permissions.checker import PermissionMode
    ...
    index_lines = [line for line in index_content.strip().split("\n") if line.strip()]
