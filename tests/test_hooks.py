...
class TestCommandExecutor:
    ...
    async def test_normal_execution(self):
        from codepilot.hooks.executors import execute_command
...
class TestPromptExecutor:
    ...
    async def test_returns_message(self):
        from codepilot.hooks.executors import execute_prompt
...
class TestHttpExecutor:
    ...
    async def test_mock_request(self):
        from codepilot.hooks.executors import execute_http

        action = Action(type="http", url="https://httpbin.org/post", body='{"test": true}')
        ctx = HookContext()
        # 用 mock 避免发起真实的网络请求
        with patch("codepilot.hooks.executors.urlopen") as mock_urlopen:
    ...
    async def test_rejects_non_http_scheme(self):
        ...
        result = await execute_http(Action(type="http", url="file:///etc/passwd"), HookContext())
...
class TestExecuteAction:
    ...
    async def test_dispatch(self):
        from codepilot.hooks.executors import execute_action
...
class TestAgentHookIntegration:
    ...
    async def test_pre_tool_use_reject_skips_tool(self):
        from codepilot.agent import Agent, ToolResultEvent
        ...
        from codepilot.tools.base import StreamEnd, TextDelta, ToolCallComplete
