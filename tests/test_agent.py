...
from codepilot.serialization import build_anthropic_messages
...
def test_set_work_dir_updates_relative_path_tools_and_permissions(tmp_path):
    ...
    checker = PermissionChecker(
        ...
        sandbox=PathSandbox(str(original)),
    ...
    agent.set_work_dir(str(target))

    assert agent.work_dir == str(target.resolve())
    assert checker.sandbox.project_root == target.resolve()
    assert all(tool.work_dir == str(target.resolve()) for tool in registry.list_tools())
...
async def test_single_step_tool_call():
    ...
    client = MockLLMClient(
        [
            ...
            [
                ...
                ToolCallComplete("t1", "ReadFile", {"file_path": "CODEPILOT.md"}),
    ...
    conv.add_user_message("Read CODEPILOT.md")
...
async def test_multi_step_autonomous():
    ...
    test_file = "/tmp/codepilot_test_hello.txt"
        ...
        [
            ...
            [
                ...
                ToolCallComplete(
                    "t1",
                    "WriteFile",
                    {"file_path": "/tmp/codepilot_test_hello.txt", "content": "Hello World"},
                ),
                StreamEnd("end_turn", input_tokens=10, output_tokens=20),
            ],
            # 第 2 轮：ReadFile 进行验证
            [
                ...
                ToolCallComplete("t2", "ReadFile", {"file_path": "/tmp/codepilot_test_hello.txt"}),
...
async def test_stop_max_iterations():
    ...
    for i in range(5):
        responses.append(
            [
                TextDelta(f"Step {i}"),
                ToolCallComplete(f"t{i}", "ReadFile", {"file_path": "CODEPILOT.md"}),
...
async def test_stop_cancel():
    ...
    class SlowMockClient(LLMClient):
        ...
        async def stream(
            ...
            yield ToolCallComplete(
                f"t{self._call_count}", "ReadFile", {"file_path": "CODEPILOT.md"}
            )
            await asyncio.sleep(0.01)
    ...
    await asyncio.wait_for(turn_observed.wait(), timeout=2)
    task.cancel()
    try:
        await task
...
async def test_token_usage_accumulates():
    ...
    client = MockLLMClient(
        [
            ...
            [
                TextDelta("Step 2"),
                ToolCallComplete("t2", "ReadFile", {"file_path": "CODEPILOT.md"}),
...
def test_partition_tool_calls():
    """分批逻辑会把可并发执行的调用归到同一组。"""
    from codepilot.tools.base import ToolCallComplete
