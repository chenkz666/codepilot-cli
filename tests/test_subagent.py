...
from codepilot.agents.parser import AgentDef, AgentParseError, parse_agent_file, parse_frontmatter
from codepilot.agents.loader import AgentLoader
from codepilot.agents.tool_filter import (
    ASYNC_AGENT_ALLOWED_TOOLS,
    resolve_agent_tools,
)
from codepilot.agents.fork import (
    FORK_BOILERPLATE_TAG,
    ForkError,
    build_forked_messages,
)
from codepilot.agents.trace import TraceManager
from codepilot.agents.task_manager import BackgroundTask, TaskManager
from codepilot.agents.notification import format_task_notification, inject_task_notifications
from codepilot.conversation import ConversationManager, ToolUseBlock
...
class TestAgentParser:
    ...
    def test_parse_default_values(self, tmp_path: Path):
        ...
        assert agent_def.max_turns == 200  # 未指定时默认 200
...
class TestAgentLoader:
    def test_load_builtins(self, tmp_path: Path):
        ...
        assert agents["Explore"].max_turns == 200  # 未指定时默认 200
    ...
    def test_project_overrides_builtin(self, tmp_path: Path):
        agents_dir = tmp_path / ".codepilot" / "agents"
...
class TestToolFilter:
    ...
    def test_builtin_no_custom_restrictions(self):
        # EnterPlanMode 现在已归入 ALL_AGENT_DISALLOWED，
        # 所以应当用一个只在 CUSTOM 而不在 ALL 中的工具，来验证内置 agent
        # 会跳过 custom 这一层。ALL 中的限制也已纳入 CUSTOM，
...
class TestPermissionMode:
    def test_bypass_mode(self):
        from codepilot.permissions.modes import PermissionMode, mode_decide
...
class TestAgentToolParams:
    def test_required_fields(self):
        from codepilot.tools.agent_tool import AgentToolParams
