...
from codepilot.config import WorktreeConfig, load_config
from codepilot.worktree.changes import count_worktree_changes, has_worktree_changes
from codepilot.worktree.integration import build_worktree_notice, generate_worktree_name
from codepilot.worktree.manager import WorktreeError, WorktreeManager
from codepilot.worktree.models import WorktreeSession
from codepilot.worktree.session import load_worktree_session, save_worktree_session
from codepilot.worktree.slug import flatten_slug, validate_slug
...
def _init_git_repo(path: Path) -> None:
    ...
    subprocess.run(
        ["git", "config", "user.email", "codepilot-tests@example.com"],
        cwd=str(path),
        capture_output=True,
    )
