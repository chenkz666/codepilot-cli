...
from codepilot.conversation import (
    ConversationManager,
    Message,
    ToolResultBlock,
    ToolUseBlock,
...
from codepilot.memory.instructions import (
    MAX_INCLUDE_DEPTH,
    load_instructions,
    process_includes,
)
from codepilot.memory.session import (
    RecordType,
    Session,
    SessionManager,
    SessionMeta,
...
class TestProcessIncludes:
    ...
    def test_path_outside_project_not_found(self, tmp_path: Path) -> None:
        """项目外路径不做路径限制；不存在的文件显示 file not found。"""
    ...
    def test_cycle_detection(self, tmp_path: Path) -> None:
        ...
        result = process_includes("@include ./a.md", tmp_path, tmp_path)
...
class TestLoadInstructions:
    def test_single_layer(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        codepilot_md.write_text("project instructions", encoding="utf-8")
    ...
    def test_multi_layer_priority(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        """发现顺序：CODEPILOT.md 在前，.codepilot/INSTRUCTIONS.md（legacy）在后。"""
        root_md = tmp_path / "CODEPILOT.md"
        root_md.write_text("root level", encoding="utf-8")
        dotdir = tmp_path / ".codepilot"
        dotdir.mkdir()
        # .codepilot/CODEPILOT.md 不作为候选，只发现 .codepilot/INSTRUCTIONS.md（legacy）
...
class TestSession:
    def test_append_writes_jsonl_and_updates_meta(self, tmp_path: Path) -> None:
        sessions_dir = tmp_path / ".codepilot" / "sessions"
...
class TestMemoryManager:
    """验证独立 .md 文件、frontmatter 和 MEMORY.md 索引格式。"""
    ...
    def test_load_includes_memory_index(
        ...
        # 创建项目级记忆目录和文件
        project_mem_dir = tmp_path / "project" / ".codepilot" / "memory"
        ...
        index_file.write_text("- [Test](test_mem.md) — a test memory\n", encoding="utf-8")
    ...
    def test_load_all(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        # 用户级记忆
        user_mem_dir = fake_home / ".codepilot" / "memory"
...
class TestConversationInjection:
    def test_inject_long_term_memory(self) -> None:
        ...
        assert "codepilotMd" in conv.history[1].content
    ...
    def test_inject_instructions_only(self) -> None:
        ...
        assert "codepilotMd" in conv.history[0].content
...
class TestMemoryExtraction:
    def test_supported_memory_types(self, tmp_path: Path) -> None:
        """验证四种记忆类型及其作用域划分。"""
        from codepilot.memory.auto_memory import (
            VALID_TYPES,
            _USER_LEVEL_TYPES,
