...
from codepilot.teams.shared_task import SharedTaskStore
from codepilot.teams.mailbox import Mailbox, create_message
from codepilot.teams.registry import AgentNameRegistry
from codepilot.teams.backend_detect import detect_backend, detect_pane_backend
from codepilot.teams.coordinator import (
    get_coordinator_system_prompt,
    get_coordinator_user_context,
    is_coordinator_mode,
    match_session_mode,
)
from codepilot.agents.tool_filter import (
...
class TestModels:
    ...
    def test_unique_team_name(self, tmp_dir):
        with patch("codepilot.teams.models.Path.home", return_value=Path(tmp_dir)):
            name1 = unique_team_name("my-team")
            assert name1 == "my-team"
            (Path(tmp_dir) / ".codepilot" / "teams" / "my-team").mkdir(parents=True)
...
class TestMailbox:
    ...
    def test_lock_contention_does_not_write_without_lock(self, tmp_dir, monkeypatch):
        mailbox = Mailbox(tmp_dir)
        lock_path = mailbox._lock_path("busy-agent")
        lock_path.write_text("held", encoding="utf-8")
        monkeypatch.setattr("codepilot.teams.mailbox.time.sleep", lambda _delay: None)

        with pytest.raises(TimeoutError, match="mailbox lock"):
            mailbox.write("busy-agent", create_message("a", "b", "must not be written"))

        assert not mailbox._inbox_path("busy-agent").exists()
...
class TestBackendDetect:
    ...
    def test_pane_iterm2_with_it2(self):
        ...
        with patch.dict(os.environ, env, clear=False):
            with patch("codepilot.teams.backend_detect.shutil.which") as mock_which:
    ...
    def test_pane_no_backend_falls_back_to_in_process(self):
        ...
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("TMUX", None)
            os.environ.pop("TERM_PROGRAM", None)
            with patch("codepilot.teams.backend_detect.shutil.which", return_value=None):
                result = detect_pane_backend()
                assert result == BackendType.IN_PROCESS
...
class TestConfigExtensions:
    def test_teammate_mode_defaults(self):
        from codepilot.config import AppConfig
...
class TestTranscript:
    def test_save_and_load(self, tmp_dir):
        ...
        from codepilot.teams.transcript import load_transcript, save_transcript
    ...
    def test_load_nonexistent(self, tmp_dir):
        from codepilot.teams.transcript import load_transcript
...
class TestAgentCoordinatorIntegration:
    def test_normal_prompt(self):
        from codepilot.prompts import build_system_prompt, IDENTITY_SECTION
    ...
    def test_coordinator_prompt(self):
        from codepilot.prompts import build_system_prompt
