...
from codepilot.commands.parser import complete, parse_command
from codepilot.commands.registry import (
    Command,
    CommandContext,
    CommandRegistry,
    CommandType,
...
class TestComplete:
    ...
    def test_completion_does_not_drop_commands_after_eight(self) -> None:
        registry = CommandRegistry()
        for index in range(12):
            registry.register_sync(_make_command(f"command-{index:02d}"))

        matches = complete(registry, "/")

        assert len(matches) == 12
        assert matches[-1][1] == "/command-11"
...
class TestHelpHandler:
    ...
    async def test_list_all(self) -> None:
        ...
        from codepilot.commands.handlers.help import handle_help
...
class TestPlanDoHandlers:
    ...
    async def test_plan_switches_mode(self) -> None:
        from codepilot.commands.handlers.plan import handle_plan
...
class TestSkillHandler:
    ...
    async def test_skill_list_no_loader(self) -> None:
        from codepilot.commands.handlers.skill import handle_skill
...
class TestStatusHandler:
    ...
    async def test_status_output(self) -> None:
        from codepilot.commands.handlers.status import handle_status
        ...
        assert "CodePilot 状态" in ui.messages[0]
...
class TestSessionHandler:
    ...
    async def test_session_no_manager(self) -> None:
        from codepilot.commands.handlers.session import handle_session
    ...
    @pytest.mark.asyncio
    async def test_session_resume_number_recomputes_candidates(self) -> None:
        from codepilot.commands.handlers.session import handle_session

        first_meta = MagicMock(id="session-first")
        second_meta = MagicMock(id="session-second")
        resumed_session = MagicMock(session_id="session-second")
        result = MagicMock(session=resumed_session, messages=[])
        result.session.meta.message_count = 0
        sm = MagicMock()
        sm.list.return_value = [first_meta, second_meta]
        sm.resume.return_value = result
        agent = MagicMock()
        ctx = _make_context(args="resume 2")
        ctx.agent = agent
        ctx.session_manager = sm
        ctx.config = {
        ...
        sm.resume.assert_called_once_with("session-second")
...
class TestMemoryHandler:
    ...
    async def test_memory_display(self) -> None:
        from codepilot.commands.handlers.memory import handle_memory
