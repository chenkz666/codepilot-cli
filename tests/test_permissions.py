...
from codepilot.tools.base import StreamEnd, StreamEvent, TextDelta, ToolCallComplete
...
class TestPathSandbox:
    ...
    def test_temp_dir_allowed(self) -> None:
        tmp = Path(tempfile.gettempdir()) / "codepilot_test.txt"
...
class TestRuleEngine:
    ...
    def test_append_local_rule(self) -> None:
        tmpdir = Path(tempfile.mkdtemp())
        local_path = tmpdir / ".codepilot" / "permissions.local.yaml"
...
class TestPermissionMode:
    ...
    # CUSTOM and DONT_ASK have been removed; only four permission modes remain.
...
async def test_e2e_rule_allows_git():
    ...
    tmpdir = Path(tempfile.mkdtemp())
    rules_file = tmpdir / ".codepilot" / "permissions.yaml"
